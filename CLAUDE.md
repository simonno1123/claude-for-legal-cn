...
marketplace — twelve default China legal plugins, optional external vendor plugins, and five
...
- Do not hand-edit module `.mcp.json` files. They are generated by `scripts/generate-mcp-configs.py`.
- Shared connector defaults belong in `scripts/mcp-template.json`.
- Module-specific connector additions belong in `scripts/mcp-modules.json` under `extraServers`.
- New modules must be registered in the `modules` array in `scripts/mcp-modules.json`.
- After any connector change, regenerate MCP configs, validate JSON, and rerun the localization residue scan.

## Layout

```
.claude-plugin/marketplace.json   # the marketplace manifest — one entry per plugin
<plugin>/                         # 12 default China legal plugins
