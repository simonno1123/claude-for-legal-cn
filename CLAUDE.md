...
在中国大陆法域下，必须丢弃美国普通法概念和美国证据开示制度的默认假设。所有技能、代理和 README 必须遵循 `references/us-to-cn-legal-terms.md`：

- 不得承诺 `attorney-client privilege`、`work product doctrine`、`discovery-proof` 或任何“证据特权/开示豁免”效果。
- `attorney work product` 只能适配为“律师工作成果初稿 / 内部法律分析 / 保密文件”。
- `attorney-client privilege` 只能适配为“律师保密义务 / 委托关系保密 / 律师执业保密义务”。
